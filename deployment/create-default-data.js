...
const TIMEMOUT_MS = 500;
let tryCounter = 0;
let doneMigration = false;
const models = [
  // Built-in
  'User',
  'AccessToken',
...
function tryConnecting() {
  tryCounter++;
  console.log(`Loop #${tryCounter}`);
  if(tryCounter <= MAX_RETRIES) {
    setTimeout( () => {
      try {
        app.models.Researcher.find({}, function(err, data){
          if (err) throw err;

          if(data.length >= 1) {
            db.autoupdate(models, function(err, result) {
              // if (err) throw err;
              console.log('Auto-Updated Following Loopback Tables [' + models + '] created in ', db.adapter.name);
              doneMigration = true;
              db.disconnect();
            });
          } else {
            db.automigrate(models, function(err) {
              // if (err) throw err;
              console.log('Auto-Migrated Following Loopback Tables [' + models + '] created in ', db.adapter.name);
              createMockData();
              // db.disconnect();
            });
          }
        ...
        // If database is down or not ready
        tryConnecting()
        console.log(err);
      }
    }, TIMEMOUT_MS);
